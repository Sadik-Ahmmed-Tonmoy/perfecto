...
const BrandPageForMobile = () => {
    ...
    const [brandList, setBrandList] = useState([...brandListForFilterQuery?.data]);
    ...
    useEffect(() => {
        if (watchSearch?.length > 0) {
            setFilteredBrandList(brandList.filter((brand) => brand?.caption.toLowerCase().includes(watchSearch?.toLowerCase())));
        } else {
